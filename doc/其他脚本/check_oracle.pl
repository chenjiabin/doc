...
#    my $cmd = "sqlplus dummy/user\@$opt_l < $null_dev";
    my $cmd = "sqlplus dummy/user\@$connect_string < $null_dev";

    if($opt_u){
#		$cmd = "sqlplus $opt_u/$opt_p\@$opt_l < $null_dev";
...
sub check_query() {
	...
	unless ($opt_a =~ /^([-_\@\"\\\$\*\(\)=<>,\'\% \w.]+)$/) {
		print "QUERY contains illegal characters\n";
		exit $ERRORS{"UNKNOWN"};
	}
	if($opt_s) {
		unless ($opt_s =~ /^([-_\@\"\%:=\(\)\' \w.]+)$/) {
			print "Output string contains illegal chars\n";
			exit $ERRORS{"UNKNOWN"};
		}
		$outStr = $opt_s;
	} else {
	...
	if($opt_r){
		unless ($opt_r =~ /^([-_\@\%:=\(\)\ \w.]+)$/) {
			print "Search string contains illegal chars\n";
			exit $ERRORS{"UNKNOWN"};
		}
		$numhits = 0;
...
sub check_datafile_bytes() {
	if($opt_a){
		# $sql_cmd = "select round(((a.BYTES-b.BYTES)/a.BYTES)*100,2), a.FILE_NAME from ( select sum(BYTES) BYTES, FILE_NAME from dba_data_files where file_id=(select max(file_id) from dba_free_space where tablespace_name='$opt_a') group by FILE_NAME ) a, ( select sum(BYTES) BYTES from dba_free_space where file_id=(select max(file_id) from dba_free_space where tablespace_name='$opt_a') group by TABLESPACE_NAME ) b ;";
		$sql_cmd = "select (MAXBYTES - BYTES), FILE_NAME FROM dba_data_files WHERE FILE_ID=(select max(file_id) from dba_data_files where tablespace_name='$opt_a');";
		@res = exec_sql_cmd($sql_cmd);
		$result = trim(@res[0]);
		# Turn bytes into MB
...
#    print "    check that database is running\n";
    ...
    print "    Compute the cache hit ratio in %. Alert if less then <warn> <crit>.\n";
