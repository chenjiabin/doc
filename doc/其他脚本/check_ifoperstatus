...
sub print_help() {
	print_revision($PROGNAME, '1.4.16.git');
	print_usage();
	...
	printf "   -a (--authproto)  Authentication protocol (MD5 or SHA)\n";
	...
	if ($snmp_version =~ /3/ ) {
		...
		if (defined $seclevel && defined $secname) {
			...
			if ( $seclevel eq 'authNoPriv' || $seclevel eq 'authPriv' ) {
				if (defined $authproto && $authproto ne 'MD5' && $authproto ne 'SHA') {
					usage("Auth protocol can be either MD5 or SHA");
				}
