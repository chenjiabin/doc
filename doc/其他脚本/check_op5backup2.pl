...
nagios_exit("$backup_dir/backup-$max_date-$max_time.log does not exists", 2) if (!-f "$backup_dir/backup-$max_date-$max_time.log");
...
nagios_exit("Latest backup scheduled on $max_date is too old(more then $opt_c days)", 2) if ($time_diff > 86400 * $opt_c);
nagios_exit("Latest backup scheduled on $max_date is too old(more then $opt_w days)", 1) if ($time_diff > 86400 * $opt_w);
